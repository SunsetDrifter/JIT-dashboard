...
**Auth:** verify forwarded OIDC JWT (`jose`, issuer/audience = dashboard's `AUTH_AUTHORITY`/`AUTH_AUDIENCE`); resolve NetBird user+role via the service token (role from NetBird, never the JWT); fail closed on ambiguity. **Scheduler:** expire, auto-deny pending, reconcile, retry; crash-safe (state in SQLite). **Security:** service-token secret validated at startup (capability probe), per-user async mutex, idempotent revoke that never fails open, rate limiting, audit log. Every Grant status change flows through a single **grant-lifecycle** transition (legal-edge table + atomic compare-and-set + derived audit; `jit-service/src/domain/grantLifecycle.ts`).
